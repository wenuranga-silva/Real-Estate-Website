...
export default function Index({ property }) {
    ...
    const submitForm = (e) => {
        ...
        setShowLoading(true);
        post(route('admin.propertyImage.store'), {
    ...
    const submitDelete = (e, imageId) => {
        ...
        setShowLoading(true);
        router.delete(route('admin.propertyImage.destroy', imageId), {
    ...
    return (
        ...
        <AdminLayout menuActive={'property'}
            topBar={
                <CustomTopBar title={'Property - Images'} _url={route('admin.property.index')}>
                    All Properties
                </CustomTopBar>
            }
        >
            ...
            <Card className="p-2 md:p-4">
                ...
                <form onSubmit={submitForm} className="mb-10 mt-5">
                    ...
                    <div className="flex justify-around mt-5 mb-10">
                        ...
                        <PrimaryButton disabled={processing} type="submit" className="w-fit">{processing ? 'Processing ...' : 'Add Property Images'}</PrimaryButton>
            ...
            <Card>
                ...
                <h2 className="mx-auto text-secondary text-lg font-_Lato">Images - {property.title}</h2>
                    ...
                    {property.property_images.map((image) => (
                        ...
                        <div key={image.id} className="relative">
                            ...
                            <button onClick={e => submitDelete(e, image.id)} className="p-3.5 text-lg absolute top-3 right-3 text-white bg-slate-600 hover:bg-slate-300/70 hover:text-red-600 transition ease-in duration-200 shadow-md rounded-full">
                                ...
                                <DeleteIcon />
                                {/* <i className="fa-solid fa-trash text-lg py-1 px-3 absolute top-3 right-3 text-white bg-slate-600/70 hover:bg-slate-200/70 hover:text-red-600 transition ease-in duration-200 shadow-md rounded-full"></i> */}
                            </button>
                        </div>
                    ))}

                    {property.property_images.length == 0 ? <code>No Images For This Property !</code> : ''}
